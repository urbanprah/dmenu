...
drawmenu(void)
{
	...
	if (passwd) {
	        censort = ecalloc(3, sizeof(text));
                memset(censort, passwordsymbol, strlen(text));
...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		if (centered) {
			...
			mw = info[i].width - 4 ; /* TODO: figure out why width is +4 */
		}
...
usage(void)
{
	fputs("usage: dmenu [-bfinPv] [-l lines] [-p prompt] [-fn font] [-m monitor] [-h height]\n" \
